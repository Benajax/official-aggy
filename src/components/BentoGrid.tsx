...
export default function BentoGrid({ socials, artist, tourDates }: any) {
  // Socials structure check (handles different API response shapes)
  ...
  // Adaptive State: Stores { url, fillContainer }
  const [selectedVideo, setSelectedVideo] = useState<{ url: string, fillContainer: boolean } | null>(null);
  ...
  useEffect(() => {
    ...
    if (pool.length > 0) {
      ...
      setSelectedVideo({
        url: picked.url,
        fillContainer: picked.fillContainer ?? true
      });
    }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 max-w-7xl mx-auto auto-rows-[200px] p-4 bg-black min-h-screen">
      ...
      {/* 3. HERO VIDEO (2x2) - ADAPTIVE PLAYER */}
      <div 
        ref={containerRef}
      ...
      >
        {selectedVideo ? (
          ...
          <div className="absolute inset-0 w-full h-full bg-zinc-900 flex items-center justify-center">
            <div className="text-white/30 animate-pulse uppercase tracking-widest text-[10px] font-black">Initialising Player...</div>
          </div>
        )}
      ...
      <div className="md:col-span-1 md:row-span-2 bg-zinc-800 rounded-[2.5rem] overflow-hidden relative border border-white/5 group">
        {posts[0] && (
          <img src={posts[0].mediaUrl} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt="Social" />
        )}
      ...
      <div className="md:col-span-1 md:row-span-2 bg-zinc-950 border border-white/5 rounded-[2.5rem] p-8 flex flex-col h-full relative overflow-hidden">
        ...
        <button
          ...
          className="relative z-10 mt-6 w-full py-4 bg-white text-black rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-cyan-400 transition-all"
        >
          Contact
        </button>
      </div>
      ...
      {[1, 2].map((idx) => (
        <div key={idx} className="md:col-span-1 md:row-span-1 bg-zinc-900 rounded-[2.5rem] overflow-hidden border border-white/5 group relative">
          {posts[idx] ? (
            <img src={posts[idx].mediaUrl} className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-all duration-500" alt="Social" />
          ) : (
